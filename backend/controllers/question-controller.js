...
export function createQuestionMcq(req, res) {
  ...
  const optionsFromBody = req.body.options;
  ...
  for (let i = 0; i < optionsFromBody.length; i++) {
    options[i] = optionsFromBody[i];
  }
  console.log(options);
  console.log(optionsFromBody);

  console.log(options, "optionsAKSMDKD");

  connection
    .promise()
    .query(
      `INSERT INTO question(question_type, question_header, correct_answer,
        ...
        VALUES('${questionType}','${questionHeader}', '${correctAnswer}',
         '${options[0].option}', '${options[1].option}', '${options[2].option}',
          '${options[3].option}', '${options[4].option}', '${options[5].option}',
           '${options[6].option}', '${options[7].option}', '${questionBankId}')`
    )
    .then((data) => {
      res.status(201).json({
        status: "ok",
...
export function createQuestionEssay(req, res) {
  ...
  connection
    .promise()
    .query(
      `INSERT INTO question (question_type, question_header,
