...
const EditPoliceSecurity = (props) => {
  ...
  const [policeNumber, setPoliceNmuber] = useState("");
  ...
  const getStudent = () => {
    axios
      ...
      .then((res) => {
        ...
        setPoliceNmuber(res.data.student[0].examinee_police_number);
  ...
  const formSubmit = () => {
    ...
    axios
      .post(
        `http://${process.env.REACT_APP_API_IP}:4000/examinee/editPoliceSecurity`,
        {
          examineeId: studentId,
          name,
          type,
  ...
  const policeNumberHandler = (e) => {
    e.preventDefault();
    setPoliceNmuber(e.target.value);
    console.log(e.target.value);
  };
...
export default EditPoliceSecurity;
